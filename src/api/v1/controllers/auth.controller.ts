...
export const login = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (!isPasswordValid)
      throw new AppError("Password is incorrect", 401, "AUTH_INVALID");
...
export const signup = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // Generate OTP before transaction to avoid timeout
    const { otp, hashedOtp } = await generateOtp();

    // Start transaction for user and OTP creation only (email sending happens after)
    ...
    // If email fails, user is already created (this is acceptable)
    try {
      ...
      // Don't fail the signup if email fails - user can request OTP resend
    }
    ...
    if (error.code === "ESOCKET" || error.syscall === "connect") {
      throw new AppError(
        "Failed to send verification email. The account was not created. Please try again later.",
        503,
        "SERVICE_UNAVAILABLE",
      );
    }
...
export const verifyEmail = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!email || !otp)
    throw new AppError("Email and OTP are required", 400, "BAD_REQUEST");

  const user = await prisma.user.findUnique({ where: { email } });

  if (!user) throw new AppError("User not found", 404, "NOT_FOUND");
  if (user.isEmailVerified === true)
    throw new AppError("User is verified already", 400, "VERIFIED_ALREADY");
...
export const resendEmailVerificationOtp = asyncHandler(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!tokenRecord) {
      try {
        ...
        const userToInvalidate = await prisma.user.findUnique({
        ...
        if (userToInvalidate) {
          ...
          console.warn(
            `⚠️ Potential refresh token reuse detected for ${decoded.UserInfo.email}. All sessions cleared.`,
          );
        }
      } catch (err) {
...
export const googleAuth = asyncHandler(async (req, res) => {
  ...
  if (!email || !sub || !given_name || !family_name) {
    throw new AppError(
      "Missing required fields from Google token",
      400,
      "BAD_REQUEST",
    );
  }
